...
flowchart TD
    ...
    subgraph DataAndTools ["5. Database, RAG, MCP & LLM Services"]
        ...
        SupervisorGraph & StudyNotesGen & MCQGen & EvalService & MapReduceEngine --> LLMFailover[LLM Fallback Chain]
        LLMFallback -->|Primary| Mistral[Mistral Small]
    ...
    subgraph ResponseDelivery ["6. Response Delivery & Export"]
        ...
        SSEStreamer & JSONResp & ExportEngine --> ReactApp
        ReactApp --> Student
    end
```
...
sequenceDiagram
    ...
    ClientJS->>API: HTTP Request with Header 'Authorization: Bearer <JWT>'
...
flowchart TD
    ...
    subgraph ExtractionWorker ["Extraction & Chunking Worker (asyncio.to_thread)"]
        ...
        FastExtract & PlumberExtract --> UnlinkTemp[Delete NamedTemporaryFile]
...
flowchart TD
    ...
    subgraph Tier1Syllabus ["Tier 1: Global Syllabus Retrieval (vector_store.py)"]
        ...
        AtlasVecSearch & InMemCosine1 & MongoTextSearch & RegexSearch --> RRF[merge_rrf: Reciprocal Rank Fusion k=60]
    ...
    subgraph Tier2UserDoc ["Tier 2: Tenant-Isolated Document Retrieval (user_doc_retriever.py)"]
        ...
        GenericQueryCheck -- Yes --> DirectSequential[_direct_chunk_fetch: Fetch chunks ordered by page_number]
        GenericQueryCheck -- No --> QEmbed2[embed_query: Dense Vector Embedding]
        
        QEmbed2 --> UserAtlasVec[_atlas_vector_search: $vectorSearch with user_id + doc_id filter]
        UserAtlasVec -.->|If Atlas search fails| UserInMemCosine[_in_memory_similarity_search: Filter by user_id & doc_id]
        UserInMemCosine -.->|If no vectors found| DirectSequential
        
        UserAtlasVec & UserInMemCosine & DirectSequential --> Tier2Ranked[Top 8 tenant-isolated chunks with page numbers]
    end

    Tier1Ranked & Tier2Ranked --> FormatContext[format_user_doc_context: Format markdown with Page Excerpts]
...
flowchart TD
    ...
    RetrieveContext & CleanWebDoc --> RouteDispatch{Execution Route}
    ...
    subgraph MapReduceRoute ["Option C: Ad-hoc PDF Map-Reduce (summarizer.py)"]
        ...
        DirectReduce & RecursiveReduce --> CohesiveSummary[Cohesive Document Overview]
    end

    ValidatedPydantic & SynthNotes & CohesiveSummary --> UIViewer[Render StudyDocumentViewer Component]
...
flowchart TD
    ...
    MergeContext & SynthReview & SafeWebError & ReturnConnGuide --> FinalResponse[Deliver response to student UI]
```
...
flowchart LR
    ...
    UploadDoc --> UserDocSvc -->|Insert chunk records & vectors| UserDocsColl
    ListDocs --> DocsRoute -->|Aggregate doc_id, filename, chunks| UserDocsColl
    DeleteDoc --> DocsRoute -->|Delete by user_id & doc_id| UserDocsColl
    
    ChatQuery --> DocRetriever -->|Query by user_id & doc_id| UserDocsColl
    ChatQuery --> VecStore -->|Hybrid RRF vector & text search| SyllabusColl
    
    LinkGH --> TokenStore -->|Upsert Fernet encrypted token| GHTokensColl
    GHAction --> GHAgent --> TokenStore -->|Find & decrypt token| GHTokensColl
```
...
flowchart TD
    ...
    subgraph LLMRateLimits ["3. LLM Rate Limits & Quotas"]
        FailurePoint -- "Google Gemini 429 Resource Exhausted" --> EmbedRetry[_embed_batch_with_retry: Regex match retryDelay]
